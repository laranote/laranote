...
import GeminiMenu from '../Components/GeminiMenu.vue'
...
export const GeminiAI = Extension.create({
    name: 'geminiAI',

    addCommands() {
        return {
            insertGeminiAI: () => ({ chain, editor }) => {
                console.log('Gemini AI command triggered');
                ...
                const app = createApp(GeminiMenu, {
                    show: true,
                    editor: editor,
                    position: {
                        x: posX,
                        y: posY + 10, // Position slightly below the cursor/click
                    },
                    close: () => {
                        setTimeout(() => {
                            app.unmount();
